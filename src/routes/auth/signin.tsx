...
import { ElectronOnly } from "@/components/miscellaneous/electron-only";
...
function RouteComponent() {
  ...
  if (isAuthenticated) return <Navigate to="/" from="/auth/signin" />;
      ...
      <Card className="w-full bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold text-center text-neutral-900 dark:text-neutral-100">
            Sign In <ElectronOnly>In Electron!</ElectronOnly>
          </CardTitle>
        ...
        <CardContent>
          <form onSubmit={handleSignIn} className="space-y-4">
            ...
            <Button
              ...
              disabled={isLoading}
              onClick={() => {
                if (isInElectron(window)) window.ipc.ping();
              }}
            >
